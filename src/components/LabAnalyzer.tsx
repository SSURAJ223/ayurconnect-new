...
const ACCEPTED_FILE_TYPES_STRING = 'image/*,application/pdf';
...
export const LabAnalyzer: React.FC<LabAnalyzerProps> = ({ personalizationData, cart, onAddToCart, onTalkToDoctorClick, isAuthenticated, openLoginModal }) => {
  const [reportData, setReportData] = useState<string>('');
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [results, setResults] = useState<LabAnalysisResult | null>(null);
  const [pendingResult, setPendingResult] = useState<LabAnalysisResult | null>(null);
  ...
  }, [reportData, selectedFile, personalizationData, isAuthenticated]);
  ...
  return (
    <div className="space-y-6">
              ...
              <div className="space-y-1 text-center">
                  ...
                  <div className="flex text-sm text-gray-600 justify-center">
                      <label htmlFor="file-upload" className="relative cursor-pointer bg-transparent rounded-md font-medium text-emerald-600 hover:text-emerald-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-emerald-500">
                          ...
                          <input id="file-upload" name="file-upload" type="file" className="sr-only" onChange={handleFileChange} accept={ACCEPTED_FILE_TYPES_STRING} capture="environment" disabled={isLoading}/>
                      </label>
      ...
      <div ref={resultsRef}>
        ...
        {pendingResult && <LoginGate openLoginModal={openLoginModal} />}
      </div>
    </div>
  );
};
